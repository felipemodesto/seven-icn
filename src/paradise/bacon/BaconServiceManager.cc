...
void BaconServiceManager::initialize(int stage) {
    ...
    switch (stage) {
        ...
        case 1: {
            ...
            //WATCH_PTRLIST(connectionList);
            //WATCH_LIST(connectionList);
        }
    }
}
...
void BaconServiceManager::startTimer(Connection_t* connection) {
    startTimer(connection, transferTimeoutTime); //Start a timer with a 1 second delay
}
    ...
    if (connection->attempts >= maxAttempts) {
        ...
        switch (connection->connectionStatus) {
            case ConnectionStatus::WAITING_FOR_NETWORK:
                {
                    //std::cout << "(SM) <" << myId << "> Re-forwarding content search after timeout <" << connection->peerID << ">\n";
                    //std::cout.flush();
            ...
            case ConnectionStatus::WAITING_FOR_ACCEPT:
                {
                    ...
                    if (connection->peerID == myId) {
                        std::cout << "(SM) <" << myId << "> Why is a local content waiting for accept again?\n";
                        std::cout.flush();
                    }
...
void BaconServiceManager::handleInterestAcceptMessage(WaveShortMessage* wsm) {
    ...
    //We've been getting this kind of message so we're filtering them now
    if (downstreamConnection->connectionStatus != ConnectionStatus::WAITING_FOR_ACCEPT) {
        //if (downstreamConnection->connectionStatus == ConnectionStatus::WAITING_FOR_NETWORK && cache->handleLookup(prefixValue) == true) {
        //    std::cerr << "(SM) <" << myId << "> Well, we could technically provide this content but our state is <" << downstreamConnection->connectionStatus << ">!\n";
        //    std::cerr.flush();
        //} else {
        //    //std::cerr << "(SM) <" << myId << "> Downstream Connection has a weird state <" << downstreamConnection->connectionStatus << ">!\n";
        //    //std::cerr.flush();
        //    //We do not have this content... We'll just ignore this request
        //}
    ...
    if (requestChunks != NULL){
        ...
        //std::cout << "(SM) <" << myId << "> should be transfering these chunks: <" << chunkString << ">\n";
    ...
    if (downstreamConnection->connectionStatus == ConnectionStatus::WAITING_FOR_ACCEPT && downstreamConnection->attempts <= 1) {
        ...
        //std::cout << "(SM) <" << myId << ">\tIncreasing UseCount for Object <" << downstreamConnection->requestPrefix << ">\tRequestID<" << downstreamConnection->requestID << ">\tpeerID<" << downstreamConnection->peerID << ">\tto<" << cache->getUseCount(downstreamConnection->requestPrefix) << ">\n";
        //std::cout.flush();
...
void BaconServiceManager::handleInterestMessage(WaveShortMessage* wsm) {
    ...
    if (downstreamConnection != NULL){
        ...
        if (downstreamConnection == NULL) {
            std::cerr <<  "(SM) <" << myId << "> WTF. Connection <" << idValue << "> with <" << wsm->getSenderAddress() << ">. Time: <" << simTime() << ">\n";
            std::cerr.flush();
            return;
        }
        ...
        //if (myId == 0) {
        //    std::cout << "(SM) Created a connection for <" << prefixValue << "> in connection <" << idValue << "> from <" << wsm->getSenderAddress() << "> Here: <" << downstreamConnection << "> ;time: <" << simTime() << ">\n";
        //    std::cout.flush();
        //}
    }
...
void BaconServiceManager::handleContentMessage(WaveShortMessage* wsm) {
    ...
    if (connection == NULL) {
        ...
        //std::cerr.flush();
        //This case means that we got the content, resolved all transmissions but something made it to our provider who sent us shit again...
        //This is kind of unnecessary but whetever
        return;
    }
...
void BaconServiceManager::notifyOfContentAvailability(WaveShortMessage* wsm, Connection_t* connection) {
    ...
    if (connection->peerID == myId) {
        ...
        connection->attempts++;
        startTimer(connection,transferTimeoutTime * (maxAttempts + 1 )); //More time than timeouts can ever take
    }
...
void BaconServiceManager::forwardContentSearch(WaveShortMessage* wsm, Connection_t* connection) {
    ...
    if (connection->upstreamHopCount > 15) {
        //connection->connectionStatus = ConnectionStatus::DONE_UNAVAILABLE;
        //std::cout << "(SM) <" << myId << "> Reached Maximum hopcount (10) for connection <" << connection->requestID << ">.\n";
        //std::cout.flush();
        delete(wsm);
        return;
    }
...
void BaconServiceManager::transmitDataChunks(Connection_t* connection, std::vector<int>* chunkVector) {
    ...
    //std::cout << "(SM) <" << myId << "> Connection <" << connection->requestID << "> to <" << connection->peerID << "> Status is now PROVIDED.\n";
    //std::cout.flush();
    ...
    startTimer(connection,transferTimeoutTime * (maxAttempts + 2)); //More time than timeouts can ever take (Including an extension based on the node receiving part of our data and postponing THEIR timer)
}
